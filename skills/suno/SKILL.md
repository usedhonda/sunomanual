...
4. **人物と関係性**: `pov` + カスタムセクション（例: 地理的スタンス）
5. **感情のズレ**: `mood` が示す感情到達点から逆算して「言えないこと」を設計
6. **具体物（モチーフ）**: `source_channels` のソースに合った具象物を選ぶ
7. **コントラスト**: `foreshadow` の設計思想に沿った対比軸を選ぶ
8. **フック**: `hooks` の設計方針 + `vocabulary` のトーンで Chorus 1行目を設計
