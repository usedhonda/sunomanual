...
```bash
# サンプルレート確認（Suno V5.5 は 192kHz で出力することがある）
...
**サンプルレート変換（44.1kHz 以外の場合）:**
配信サービス（UnitedMasters 等）は 44.1kHz のみ受付。192kHz 等の場合は Pass 3 の前に変換する:
```bash
ffmpeg -i "<input.wav>" -ar 44100 "<input_44k.wav>"
# 以降の処理は <input_44k.wav> を使用
```
