...
**ユーザーの回答が十分なら、早めに切り上げる。** 全部聞く必要はない。推測できるところはAIが埋めて「こんな感じですか？」と確認する。

### アーティストプロファイル出力

対話が終わったら YAML 形式で出力し、ファイルとして保存する。
...
production:
  instruments: ["<楽器1>", "<楽器2>"]
  mix: "<ミックスの方向性>"
  reference_sound: "<音の参照（アーティスト名ではなく音の特徴で）>"
...
**保存:** ユーザーに保存先を確認し、`Write` ツールで保存。デフォルト: `./artists/<name>.yaml`（作業フォルダ基準）
